...
function EditIcon({ navigation }: { navigation: any }) {
  ...
  const [chosenIcon, setChosenIcon] = useState<string>(
    activeAccount?.icon?.icon_value || accountData?.icon?.icon_value || "credit-card-outline"
  );

  const handleSubmit = (e: any) => {
    e.preventDefault();
    e.stopPropagation();
    console.log("chosenIcon", chosenIcon, "color", chosen);
  ...
  const Item = ({ item, onPress }: { item: string; onPress: () => void }) => (
    <TouchableOpacity
      onPress={onPress}
      style={
        chosenIcon !== item
    ...
    >
      <MaterialCommunityIcons name={item as any} size={24} color="white" />
    </TouchableOpacity>
  );
